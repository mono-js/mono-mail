...
module.exports = function ({ conf, log }) {
	const provider = (conf.mono.mail.hasProvider) ? require(`./providers/${conf.mono.mail.provider.name}.provider`) : null
	...
	async function send(mail) {
		log.info(`Sending mail ${mail.path} with context ${JSON.stringify(mail.data)}`)
